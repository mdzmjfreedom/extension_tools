$(document).ready(function () {
    ...
    $('input[name="content-type"]').change(function () {
        ...
        if (contentType === 'application/json') {
            ...
            $('#format-json-btn').removeClass('hidden'); // 显示 Format JSON 按钮
        } else {
            $('#format-json-btn').addClass('hidden'); // 隐藏 Format JSON 按钮
        }
    });
    ...
    $('#format-json-btn').click(function () {
        ...
        if (body) {
            try {
                ...
                alert('Error: Invalid JSON format'); // 简单错误提示
            }
        }
    });

    // Download response button click
    $('#download-response-btn').click(function () {
        const responseText = $('#response-body').text();
        const contentType = $('#response-status').hasClass('success') && $('#response-body').is(':visible')
